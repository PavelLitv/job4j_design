...
class CSVReaderTest {
    ...
    @Test
    void whenFileFromPathNotExist() {
        String target = "target.csv";
        String notExist = "/nothing_data/data_not_exist.csv";
        ArgsName argsName = ArgsName.of(new String[]{
                "-path=" + notExist, "-delimiter=,",
                "-out=" + target, "-filter=education,age,last_name"
        });
        assertThatThrownBy(() -> CSVReader.validate(argsName))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("Not exist: " + notExist);
    }

    @Test
    void whenOutIsNotFile(@TempDir Path folder) throws IOException {
        File file = folder.resolve("source.csv").toFile();
        file.createNewFile();
        ...
        assertThatThrownBy(() -> CSVReader.validate(argsName))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("Param 'out' not equals 'stdout' or not a path to file: " + target);
    }
}
